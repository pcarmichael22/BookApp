...
app.post('/book/:book_id/update', updateBook);
  ...
  superagent.get(bookUrl).then(result => {
    ...
    const myData = bookList.map(bookData => {
      ...
      return new Book(bookData.volumeInfo.title,
        ...
        bookData.volumeInfo.imageLinks && bookData.volumeInfo.imageLinks.smallThumbnail.replace(regex, 'https'),
        bookData.volumeInfo.industryIdentifiers && bookData.volumeInfo.industryIdentifiers[0].type
      );
    })
...
function updateBook(req, res) {
  const id = req.params.book_id;
  // title thumbnail description category author
  console.log(id);
  client.query(' \
    UPDATE books \
    SET title=$2, author=$3, description=$4, bookshelf=$5, thumbnail=$6 \
    WHERE id = $1;', [
    id, 
    req.body.title,
    req.body.author,
